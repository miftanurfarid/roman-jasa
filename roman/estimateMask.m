function generate_mask(input_directory, D, Region)
%Compute estimated binary mask from from itd/iid features using the learned decisionrules

N=Region(5,1);

inputfile_itd=strcat(input_directory,'/itd');
inputfile_iid=strcat(input_directory,'/iid');
outputfile_mask=strcat(input_directory,'/mask')


iid=load(inputfile_iid);iid=reshape(iid,128,length(iid)/128);
itd=load(inputfile_itd);itd=reshape(itd,128,length(itd)/128);
