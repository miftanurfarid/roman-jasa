...
w[ind]=0.5-0.5*cos(2.0*ind*M_PI/(n-1.0));
  ...
  ///find peak location
    ...
    {
    if(cov[i]>max_value){max_ind=i;max_value=cov[i];}
    }
...
///reference ITD for target
	...
	for (chan=0;chan<MAX_CHANNEL;chan++)
 	...
 	for (chan=0;chan<MAX_CHANNEL;chan++)
      ...
      if (index1[chan]<0) index1[chan]=0;
      index2[chan]=(int)(loc[chan]+pi);
      if (index2[chan]>2*MAX_DISP) index2[chan]=2*MAX_DISP;
...
fclose(output_ITD);fclose(output_IID);fclose(output_Ratio);
fclose(itd_file);fclose(iid_file);fclose(ratio_file);
fclose(ofp_left);fclose(ofp_right);
fclose(ofp_n);fclose(ofp_t);
}
